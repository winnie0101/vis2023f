...
function _simple1(d3,simple,drag,url_list,invalidation)
{
  ...
  const colorScale = d3.scaleOrdinal(d3.schemeTableau10); // 使用 D3 的顏色比例尺
  ...
  const node = svg.append("g")
      ...
      .on("mouseover", handleMouseOver) // 添加滑鼠移入事件處理程序
      .on("mouseout", handleMouseOut); // 添加滑鼠移出事件處理程序
  ...
  const imagesGroup = node.append("g").attr("class", "outer-apples"); // 新增一個 <g> 元素用於包裝圖片
 ...
 // 設定節點初始位置在畫布的中間
  ...
  function handleMouseOver(event, d) {
      ...
      d3.select(this).select(".outer-apples")
          .transition()
          .duration(200)
