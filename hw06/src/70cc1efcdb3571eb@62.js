...
Plot.plot({
  ...
  color: {
    ...
    range: ['#8c510a', '#35978f'],  // 你可以根據需要更改顏色
    legend: true
  },
  marks: [
  ...
  series.forEach((serie) => {
      ...
      bars.enter().append("rect")
          ...
          .on("mouseover", function(d) {
            if(colorScale(serie.key)=="#8c510a"){
                d3.select(this).attr("fill", "#f6e8c3");
            } else if(colorScale(serie.key)=="#35978f") {
                d3.select(this).attr("fill", "#c7eae5");
            }
            })
